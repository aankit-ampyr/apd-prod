...
async def get_solar_array(
    ...
    return solar_array, solar_source.year, solar_config.total_generation
...
async def get_constant_simulation_params(
    simulation_id: int, db: AsyncSession
) -> tuple[SimulationParams, list[int], bool, int]:
    # Load Profile Data
    ...
    # Solar Profile
    solar_array, year, total_solar = await get_solar_array(
    ...
    return simulation_params, bess_config.container_types, dg_config.is_included, year


async def get_possible_simulation_comb(
    simulation_id: int, container_types: list[int], dg_enable: bool, db: AsyncSession
):
    ...
    bess_db_config = bess_dg_result.scalar_one_or_none()
    if not bess_db_config:
        raise ValueError(
            f"BessDgSizingConfiguration not found for simulation {simulation_id}"
        )

    grid = np.meshgrid(
        np.arange(bess_db_config.bess_min, bess_db_config.bess_max + 5, 5),
        np.array(container_types) * 2,
        np.arange(
            bess_db_config.dg_min or 0,
            (bess_db_config.dg_max or 0) + (bess_db_config.dg_step_size or 1),
            (bess_db_config.dg_step_size or 1),
        )
        if dg_enable
        else [0],
        indexing="ij",
...
async def run_sizing_simulation(
    ...
    try:
        ...
        for i, item in enumerate(combinations, start=1):
            ...
            simulation_result.finalize()
            filtered_result = ScenarioResult(**simulation_result.model_dump())
        ...
        if simulation_job:
            try:
                simulation_job.status = SimulationJobStatus.FAILED
                simulation_job.simulation.status = SimulationStatus.FAILED
                await db.commit()
...
async def run_simulation_single_config(
    ...
    try:
        ...
        (simulation_params, _, _, year) = await get_constant_simulation_params(
            simulation_id=simulation_id, db=db
        )
...
async def run_multi_year_projection(
    ...
    try:
        ...
        for i in range(1, duration + 1):
            ...
            sim_result = MultiYearProjectionResult(
