 ...
 */
export async function fetchBlobFromApi(url: string, params?: any, filename?: string,  paramsSerializer?: (params: any) => string | undefined): Promise<Blob> {
  try {
...
export async function fetchAndDownloadBlob(args: FetchAndDownloadBlobParams): Promise<void> {
  const {url, filename, params, paramsSerializer} = args;
  try {
    const blob = await fetchBlobFromApi(url, params, filename, paramsSerializer);
