...
export const CustomConfiguration = ({setIsStepsHidden}: CustomConfigurationProps) => {
  ...
  const runSuccess = useSelector(customRunSuccess);
  ...
  const {handleRunSimulation}: any = useCustomSimulation({
  ...
  const stats = hasGenerator
    ? [
        ...
        {label: 'Green Energy (%)', value: customSimulResult?.data?.green_percentage ?? '-'},
  ...
  const isInitialLoading = isCustomConfigLoading || !customData || !simulProgressData;
  ...
  const shouldShowEmptyState1 = simulProgressData?.status !== 2;
  ...
  return (
    <div className="main">
      <div className="flex gap-3">
        <div className="bg-[#F7FDFC] p-4 mt-5 border-[1.4px] border-[#B6D7D3] rounded-lg w-[70%]">
          ...
          <div className="flex flex-col xl:flex-row gap-4 mt-1">
            <div className="bg-white p-4 mt-5 border-[1.4px] border-border rounded-md w-full xl:w-[60%] tabular-nums">
              ...
              <Text variant={'14R'} className="text-text-secondary! mt-1!">
                Duration Class:
              </Text>
              <div className="flex gap-6 mt-2">
                <Radio checked={durationClass === '2-hour'} onCheckedChange={() => handleDurationClassChange('2-hour')} label="2-hour (0.5C)" size="sm" />
                <Radio checked={durationClass === '4-hour'} onCheckedChange={() => handleDurationClassChange('4-hour')} label="4-hour (0.25C)" size="sm" />
              </div>
              <div className="mt-4 w-full">
                <TextInput
                  ...
                  decrementDisabled={bessCapacity <= 5}
                  incrementDisabled={bessCapacity >= 1000}
                />
              </div>
            </div>
            ...
            <div className="bg-white p-4 mt-5 border-[1.4px] border-border rounded-md w-full xl:w-[60%] tabular-nums">
              ...
              <div className="mt-4 w-full">
                <TextInput
                  ...
                  labelClassName="text-text-secondary! font-InterMedium!"
                  value={hasGenerator ? String(dgCapacity) : ''}
                  placeholder={hasGenerator ? undefined : 'DG not enabled in setup'}
                  readonly={hasGenerator}
                  readOnlyClassName="bg-white border-1!"
                  integer
                  min={0}
                  max={200}
                  ...
                  className="w-full! tabular-nums"
                  decrementDisabled={!hasGenerator || dgCapacity <= 0}
                  incrementDisabled={!hasGenerator || dgCapacity >= 200}
                />
                {!hasGenerator && (
                  <Text variant="12M" className="mt-2 text-text-secondary! font-InterMedium!">
        ...
        <div className="w-[30%] mt-5 rounded-2xl p-6 shadow-sm border-l-4 border-primary!">
      ...
      {(showSimulationResults || customResultSuccess === 'S-20038') && simulationCompleted && (
        <>
          <div className="rounded-2xl border border-gray-300 bg-white p-6">
            ...
            <div className="grid grid-cols-4 gap-x-6 gap-y-6.5">
              {stats?.map(item => (
                <div key={item?.label}>
