...
class SuperAdminAuditService:
    ...
    async def get_audit_logs(self, db_user, page=1, limit=100, **filters):
        try:
            ...
            async with SessionBESS() as db_bess, SessionAMD() as db_amd:
                ...
                for session, env in targets:
                    ...
                    if filters.get("module"):
                        ...
                        query = query.where(AuditLog.module.in_(mod_val) if isinstance(mod_val, list) else AuditLog.module == mod_val)

                    if filters.get("action"):
                        act_val = filters["action"]
                        query = query.where(AuditLog.action.in_(act_val) if isinstance(act_val, list) else AuditLog.action == act_val)
                    ...
                    if filters.get("start_date"):
                        ...
                        start_date = start_date - timedelta(hours=5, minutes=30)  # Convert to UTC
                    ...
                    if filters.get("end_date"):
                        ...
                        end_date = end_date - timedelta(hours=5, minutes=30)  # Convert to UTC
                    ...
                    for log in logs:
                        ...
                        after_val = log.after
                        if (not before_val or before_val == "") and after_val and "->" in after_val:
                            try:
                                before_parts = []
                                after_parts = []
                                ...
                                for change in changes:
                                    if "->" in change and ":" in change:
                                        ...
                                        before_parts.append(f"{key.strip()}: {old_val.strip()}")
                                        after_parts.append(f"{key.strip()}: {right.strip()}")
                                
                                before_val = ", ".join(before_parts) if before_parts else before_val
                                after_val = ", ".join(after_parts) if after_parts else after_val
                            except Exception:
                                pass
                        ...
                        all_logs.append({
                            ...
                            "module": {
                                'id': log.module,
                                'name': AuditLogModules(log.module).name if log.module in AuditLogModules._value2member_map_ else "UNKNOWN"
                            },
                            "action": {
                                'id': log.action,
                                'name': AuditLogScenario(log.action).name if log.action in AuditLogScenario._value2member_map_ else "UNKNOWN"
                            },
                            "resource_id": log.resource_id or "",
                            "before": before_val or "",
                            "after": after_val or "",
                            "timestamp": log.created_at.isoformat(),
                            "raw_date": log.created_at,
                        })

            all_logs.sort(key=lambda x: x['raw_date'], reverse=True) #we are sorting by raw date because if you will sort by string it will have some confusion 
            ...
            return Res.success("S-10014", data={
                "logs": paginated_logs,
