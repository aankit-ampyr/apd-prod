...
from constants.dependency_class import ModoIndustryConfig, TBSpreadBenchmark, YearlyHardcodedMetricValues
...
from worker.tasks import asset_computation_task
...
class MetricService:
    ...
    async def update_benchmarks(
        ...
        for item in payload:
            ...
            industry_low = (
                ...
                else benchmark.industry_low
            ) or 0
            industry_mid = (
                item.industry_mid
                if item.industry_mid is not None
                else benchmark.industry_mid
            ) or 0
            industry_high = (
                item.industry_high
                if item.industry_high is not None
                else benchmark.industry_high
            ) or 0
            ...
            if (industry_mid > industry_high):
                return Res.error("E-10206", message="industry_mid cannot be greater than industry_high", http_status_code=422)

            if (industry_low > industry_high):
                return Res.error("E-10207", message="industry_low cannot be greater than industry_high", http_status_code=422)
            ...
            audit_section = benchmark.metric.metric_name or "BENCHMARK CONFIGURATION"
            ...
            await audit_logs(
                ...
                module=AuditLogModules.SETTINGS,
                action=AuditLogScenario.BENCHMARK_CONFIGURATION_UPDATED,
                before={audit_section: before},
                after={audit_section: after},
        ...
        if AssetMetrics.MODO_BENCHMARK in ids:
            backgroundTask.add_task(
                asset_computation_task.kiq,
                asset_id='all',
                month='all',
                year='all',
                dependencies=[ModoIndustryConfig.__name__],
            )

        return Res.success(
            "S-10043", data=updated_records, message="Benchmarks updated successfully"
        )

    async def get_monthly_hardcoded_values(
        self, db: AsyncSession, month: List[int], year: List[int]
    ):
        ...
        return Res.success(
            "S-10045",
            data={"metrics": metrics_header, "monthly_values": monthly_values},
        )

    async def save_monthly_hardcoded_values(
        self,
        db: AsyncSession,
        redis: Redis,
        ...
        metric_display_map = [
            x.value for x in ASSET_BENCHMARK_MONTHLY_HARDCODED_METRICS
        ]
        validation_errors = []

        if validation_errors:
            return Res.error("E-10210", message="Monthly hardcoded values validation failed.", data={"errors": validation_errors}, http_status_code=422)
        ...
        for item in payload:
            if hasattr(item, "id"):
                stmt = select(MonthlyHardcodedValue).where(
                    MonthlyHardcodedValue.id == item.id
                )
            else:
                stmt = select(MonthlyHardcodedValue).where(
            ...
            updated_records_data.append(
                {
                    "id": target_record.id,
                    "metric_name": ASSET_METRIC_LABEL.get(
                        target_record.metric_id, "Unknown"
                    ),
        ...
        backgroundTask.add_task(
            ...
            year='all',
            dependencies=[
                YearlyHardcodedMetricValues.__name__,
                TBSpreadBenchmark.__name__,
            ],
        )
    ...
    async def get_modo_energy_monthly_benchmark(self, month: int, year: int):
        ...
        try:
            ...
            params = {"month_from": param_string, "month_to": param_string}

            headers = {"X-Token": MODO_ENERGY_TOKEN}
            ...
            if not entry:
                return Res.error('E-10122', message='entry not found', http_status_code=404)
            
            # caulcate benchmark value
            revenue_permw = entry["revenue_permw"]
            benchmark_value = revenue_permw * 12  # make it per year
            return Res.success(data={"modo_benchmark_per_mw_per_year": benchmark_value})
