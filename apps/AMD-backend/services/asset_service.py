...
class AssetService:
    ...
    def _score_solar_sheet(self, sheet_name: str, preview_text: str, canonical: str):
        ...
        if canonical == "Active Power 1 min":
            ...
            if "fleet sum" in text or "power ac" in text or "active power" in text:
                score += 3
            # Prefer the 1-minute source. Other power sheets are aggregates used
            # for different purposes and should not win when AP 1-min exists.
    ...
    def _resolve_solar_sheets(self, xl):
        resolved = {}
        candidates = []
        for sheet_name in xl.sheet_names:
            try:
                preview = pd.read_excel(
                    xl, sheet_name=sheet_name, header=None, nrows=8
                )
                preview_text = " ".join(
    ...
    def _find_irradiance_cols(self, df):
        ...
        # Some site exports have broken irradiance headers (for example "-1").
        # Fall back to numeric W/m2-like columns so uploads do not fail solely
        # because a sensor label is malformed.
    ...
    def _read_solar_sheet(self, xl, sheet_name):
        ...
        # header. Scan the first few rows and accept the first header that gives
        # us a recognizable timestamp column.
    ...
    def _validate_solar_dataset(self, dataset):
        errors = []
        ...
        reconciliation_diff = (
            dataset["Offpeak_kWh"].fillna(0)
            + dataset["Peak_kWh"].fillna(0)
            - dataset["Export_kWh"].fillna(0)
        ).abs()
        reconciliation_tolerance = dataset["Export_kWh"].fillna(0).abs().mul(0.001)
        reconciliation_tolerance = reconciliation_tolerance.clip(lower=0.5)
        mismatch_count = int((reconciliation_diff > reconciliation_tolerance).sum())
        if mismatch_count:
            errors.append(
                {
                    "column": "Export reconciliation",
                    "row": None,
                    "message": f"{mismatch_count} rows where off-peak + peak does not reconcile to total export.",
                }
            )
    ...
    def _build_solar_dataset(self, sheets):
        ...
        meter_export_col = self._find_solar_column(
            meter, self.SOLAR_COLUMN_ALIASES["export"]
        )
        meter_offpeak_col = self._find_solar_column(
            meter, self.SOLAR_COLUMN_ALIASES["offpeak"]
        )
        meter_peak_col = self._find_solar_column(
            meter, self.SOLAR_COLUMN_ALIASES["peak"]
        )
        spine = pd.DataFrame(
            {
                "Timestamp": pd.to_datetime(meter[meter_ts_col], errors="coerce"),
                "Export_kWh": pd.to_numeric(meter[meter_export_col], errors="coerce"),
                "Offpeak_kWh": pd.to_numeric(
                    meter[meter_offpeak_col], errors="coerce"
                ),
        ...
        wms["merge_key"] = pd.to_datetime(wms[wms_ts_col], errors="coerce").dt.floor(
            "15min"
        )
        wms["Irradiance_Wm2"] = (
        ...
        power["merge_key"] = pd.to_datetime(
            power[power_ts_col], errors="coerce"
        ).dt.floor("15min")
        power["AC_Power_kW"] = (
    ...
    async def upload_solar_report(
        ...
        try:
            ...
            try:
                xl = pd.ExcelFile(BytesIO(file_bytes), engine="openpyxl")
                resolved_sheets = self._resolve_solar_sheets(xl)
                sheets = {}
                for canonical_name, workbook_sheet_name in resolved_sheets.items():
                    sheets[canonical_name] = self._read_solar_sheet(
                        xl, workbook_sheet_name
                    )
            ...
            dataset_validation_errors = self._validate_solar_dataset(dataset)
            if dataset_validation_errors:
                formatted_errors = [
                    f"{err['column']} (Row {err['row']}): {err['message']}"
                    for err in dataset_validation_errors
                ]
                return Res.error(
                    "E-10234",
                    data={
            ...
            if asset.current_step < AssetSteps.UPLOAD_AGGREGATOR_SCADA.value:
