...
export const WebSocketProvider: React.FC<PropsWithChildren> = props => {
  ...
  const showGlobalToast = useCallback((message: string, type: ToastType) => {
    showToast(message, type);
  }, [showToast]);

  const socketManagerRef = useRef<SocketManager>(
    new SocketManager(dispatch, showGlobalToast, setIsConnectionOpen)
  );
  ...
  const getWsUrl = useCallback(async (): Promise<string | null> => {
    ...
    let socketBaseUrl = API.currentEnv ? API.baseUrls[API.currentEnv as keyof typeof API.baseUrls] : API.baseUrls.dev;
    
    // Convert HTTP to WS (and HTTPS to WSS) to comply with official WebSocket standards
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      if (socketManagerRef.current.isConnecting()) return;
      if (socketManagerRef.current.isOpen()) return;
