...
export type RechartsLineChartConfig<T extends ChartRow> = {
  ...
  transformLineData?: (data: T[], key: Extract<keyof T, string>) => Array<number | null>;
  tooltipTitleFormatter?: (value: string | number, row?: T) => string;
  tooltipValueFormatter?: (value: number, seriesName: string, row?: T) => string;
...
function alignToStep(value: number, step: number, direction: 'floor' | 'ceil'): number {
  const scaled = value / step;
  const aligned = direction === 'floor' ? Math.floor(scaled) * step : Math.ceil(scaled) * step;

  return roundToPrecision(aligned);
}

function buildYAxisTickRange(values: number[], step: number): YAxisTickRange | null {
  ...
  const ticks: number[] = [];
  for (let current = minTick; current <= maxTick + safeStep / 1000; current += safeStep) {
...
function LineActiveDot({cx, cy, color}: LineActiveDotProps) {
  ...
  return (
    <g>
      ...
      <circle cx={cx} cy={cy} r={8} fill={color} stroke={color} strokeWidth={2} />
      <circle cx={cx} cy={cy} r={4} fill="#fff" />
    </g>
  );
}

export function LineChart<T extends ChartRow>(props: RechartsLineChartProps<T>) {
  const {
    data,
    config,
    downloadFileName,
  ...
  function buildChartData<T extends ChartRow>(
    ...
    const lineSeries = lines.map(line => toNumericSeries(data, line.key, transformLineData));
  ...
  const resolvedReferenceLines = useMemo(
    () => (config.referenceLines ?? []).filter(line => Number.isFinite(line.value)),
  ...
  const chartData = useMemo(
    () => buildChartData(data, resolvedLines, config.xKey, config.transformLineData),
    [config.transformLineData, config.xKey, data, resolvedLines],
  );

  const visibleData = zoomedRange ? chartData.slice(zoomedRange.startIndex, zoomedRange.endIndex + 1) : chartData;
  ...
  }, [chartData, config.yTickValues, resolvedLines, resolvedReferenceLines, yTickStep]);
  ...
  const yDomain: [number, number] | undefined = resolvedYAxisTickRange?.domain
    ? [
        roundToPrecision(resolvedYAxisTickRange.domain[0] - yDomainBottomPadding),
  ...
  function handleChartMouseMove(state: ChartMouseState | undefined) {
    if (!dragStateRef.current.isDragging || dragStateRef.current.startIndex == null) {
      return;
    }
  ...
  function handleChartMouseUp() {
    ...
    if (selection) {
      logDragSelection(selection.startIndex, selection.endIndex, selection.startValue, selection.endValue);
  ...
  const referenceX1 = zoomedRange ? dragSelection?.startValue : dragSelection?.startIndex;

  const referenceX2 = zoomedRange ? dragSelection?.endValue : dragSelection?.endIndex;
  ...
  return (
    <div
      ...
      <div className={cn('flex items-center justify-between gap-4 px-6', headerClassName)}>
        {headerRenderer ??
          (title && (
            <Text variant="h4" className={cn('text-text-primary!', titleClassName)}>
              {title}
            </Text>
          ))}

        {!isLoading && (
          <div className={cn('flex shrink-0 items-center gap-3 chart-actions flex-nowrap', actionWrapperClassName)}>
            {customActions}
            {enableZoom && zoomedRange && (
              <button
                ...
                <Icon name="rotateCcw" className="text-text-primary! size-3.5" />
      ...
      <div className={cn('w-full grow select-none', chartClassName)} style={{height}}>
        <WithFallback
          isLoading={isLoading}
          fallback={
            <div className="px-4 flex-col pb-4 flex h-full min-h-90">
              <div className="flex items-start grow pt-2">
                <Skeleton animation="wave" variant="rounded" width={20} height={88} className="rounded-full!" />
                <div className="flex flex-col grow gap-4 my-2 mx-6">
                  <Skeleton animation="wave" variant="rounded" width="100%" height={220} />
                  <div className="flex justify-between gap-3">
                    <Skeleton animation="wave" variant="rounded" width={72} height={18} className="rounded-full!" />
                    <Skeleton animation="wave" variant="rounded" width={96} height={18} className="rounded-full!" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ReLineChart
              ...
              onMouseDown={enableZoom ? (handleChartMouseDown as any) : undefined}
              onMouseMove={enableZoom ? (handleChartMouseMove as any) : undefined}
              onTouchStart={enableZoom ? (handleChartMouseDown as any) : undefined}
              onTouchMove={enableZoom ? (handleChartMouseMove as any) : undefined}>
              {enableZoom && dragSelection && referenceX1 != null && referenceX2 != null && (
                <ReferenceArea
              ...
              {resolvedReferenceLines.map((referenceLine, index) => {
                const lineColor = referenceLine.color || LINE_COLORS[index % LINE_COLORS.length];
                ...
                return (
                  <ReferenceLine
                    ...
                    stroke={lineColor}
                    strokeWidth={2}
                    strokeDasharray={isDotted ? DOTTED_STROKE_DASHARRAY : undefined}
              ...
              <defs>
                {resolvedLines.map((lineConfig, index) => {
                  const lineColor = lineConfig.color || LINE_COLORS[index % LINE_COLORS.length];

                  const gradientStart = lineConfig.gradientColor?.start || lineColor;

                  const gradientEnd = lineConfig.gradientColor?.end || lineColor;
                  ...
                  return (
                    <linearGradient
                      ...
                      x2="0"
                      y2="1">
                      <stop offset="0%" stopColor={gradientStart} stopOpacity={lineConfig.areaOpacity ?? 0.4} />
              ...
              <CartesianGrid vertical stroke="var(--color-border)" strokeDasharray="4 4" {...cartesianGridProps} />
              <XAxis
                dataKey={config.xKey}
                ...
                content={tooltipProps => {
                  ...
                  return (
                    <LineTooltip
                      ...
                      payload={(tooltipProps.payload ?? []) as ReadonlyArray<LineTooltipPayload<T>>}
                  ...
                  content={() => (
                    <div className="flex justify-end gap-8 pb-4">
                      {resolvedLines.map((lineConfig, index) => {
                        ...
                        return (
                          <div key={String(lineConfig.key)} className="flex items-center gap-2">
                            <div className="relative w-9 h-3" aria-hidden="true">
                              <span
                      ...
                      {resolvedReferenceLines.map((referenceLine, index) => {
                        ...
                        const resolvedLegendLabel =
                          referenceLine.legendLabel ?? referenceLine.label ?? String(referenceLine.value);
                        ...
                        return (
                          <div
                            ...
                            {typeof resolvedLegendLabel === 'string' || typeof resolvedLegendLabel === 'number' ? (
                              <span
                                style={{
                                  color: lineColor,
                                  fontFamily: 'Inter-Regular',
              ...
              {resolvedLines.map((lineConfig, index) => {
                const lineColor = lineConfig.color || LINE_COLORS[index % LINE_COLORS.length];
                const lineStrokeWidth = lineConfig.strokeWidth ?? config.lineStrokeWidth ?? 2;
                return (
                  <Fragment key={String(lineConfig.key)}>
                    ...
                    <Line
                      ...
                      type={lineConfig.smooth ? 'natural' : 'linear'}
                      dataKey={lineConfig.key}
                      name={lineConfig.label || config.formatLabel?.(String(lineConfig.key)) || String(lineConfig.key)}
                      stroke={lineColor}
      ...
      {!isLoading && enableZoom && (
        <div className="px-6 pb-4 flex justify-center">
          <Text variant="14M" className="text-text-secondary!">
            Tip: click and drag horizontally to zoom into a date range. Use Reset zoom to restore.
          </Text>
        </div>
      )}
    </div>
